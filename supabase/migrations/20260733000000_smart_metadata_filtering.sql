-- Update hybrid_search_chunks to support "all" bypass for metadata tags
...
rrf AS (
    SELECT 
        ...
        COALESCE(1.0 / (60 + ROW_NUMBER() OVER(ORDER BY v.similarity DESC)), 0.0) +
        COALESCE(1.0 / (60 + ROW_NUMBER() OVER(ORDER BY f.rank DESC)), 0.0) AS rrf_score
