...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    if (request.params.name === "get_latest_notices") {
        ...
        if (error)
            return { content: [{ type: "text", text: `Error fetching notices: ${error.message}` }], isError: true };
        return { content: [{ type: "text", text: JSON.stringify(data, null, 2) }] };
    }
    if (request.params.name === "get_timetable") {
        const email = request.params.arguments?.student_email;
        if (!email)
            return { content: [{ type: "text", text: "student_email is required" }], isError: true };
    ...
    if (request.params.name === "get_student_grades" || request.params.name === "get_fee_balance" || request.params.name === "get_library_loans") {
        const regNo = request.params.arguments?.registration_number;
        if (!regNo)
            return { content: [{ type: "text", text: "registration_number is required" }], isError: true };
        if (!univPool) {
            return {
