    ...
    <div className="md-body">
      <ReactMarkdown
        ...
        components={{
          ...
          a: ({ href, children }) => <a href={href} target="_blank" rel="noopener noreferrer" style={{ color: "#f97316", textDecoration: "underline" }}>{children}</a>,
        }}
      >
        {content}
      </ReactMarkdown>
...
export function ParentWorkspace() {
        ...
        {navigation.map(([label, Icon]) => {
          ...
          return (
            <motion.button whileHover={{ scale: isActive ? 1 : 1.02 }} whileTap={{ scale: 0.98 }} key={label} onClick={() => switchTab(label)}
              style={{ display: "flex", width: "100%", alignItems: "center", gap: 12, borderRadius: 12, padding: "12px 14px", fontSize: 14, fontWeight: 500, background: isActive ? "rgba(249, 115, 22, 0.15)" : "transparent", color: isActive ? "#f97316" : "#a1a1aa", border: isActive ? "1px solid rgba(249, 115, 22, 0.2)" : "1px solid transparent", cursor: "pointer", marginBottom: 4 }}
            >
              <Icon size={18} />
              {label}
            </motion.button>
  ...
  return (
    <main className="bg-aurora" style={{ display: "flex", height: "100vh", width: "100%", overflow: "hidden", color: "#ececec", backgroundColor: "#06080a" }}>
      <style>{`
        ...
        <header className="mobile-only" style={{ height: 70, display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 20px", flexShrink: 0, zIndex: 20 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            ...
            <div style={{ display: "flex", flexDirection: "column" }}>
              <span style={{ fontSize: 20, fontWeight: 700, color: "#f97316", letterSpacing: "-0.02em", lineHeight: 1 }}>KiliGuide</span>
          ...
          <div style={{ display: "flex", gap: 10 }}>
            <button onClick={()=>switchTab("Notices")} style={{ padding: 10, borderRadius: 12, background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.1)", color: "#ececec", position: "relative" }}>
              <Bell size={20} />
              <div style={{ position: "absolute", top: 8, right: 8, width: 8, height: 8, background: "#f97316", borderRadius: "50%", border: "2px solid #06080a" }} />
            </button>
      ...
      <nav className="mobile-only" style={{ position: "fixed", bottom: 0, left: 0, right: 0, height: 80, background: "#0B0F14", borderTop: "1px solid rgba(255,255,255,0.05)", zIndex: 100, display: "flex", alignItems: "center", justifyContent: "space-around", paddingBottom: "env(safe-area-inset-bottom)" }}>
        {[
          ...
          return (
            <button key={item.id} onClick={() => switchTab(item.id)} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 6, background: "transparent", border: "none", color: isActive ? "#f97316" : "#8e8ea0", position: "relative", width: 60 }}>
              {isActive && <div style={{ position: "absolute", top: -16, width: 40, height: 3, background: "#f97316", borderRadius: "0 0 4px 4px", boxShadow: "0 4px 12px rgba(249,115,22,0.5)" }} />}
              <item.icon size={22} style={{ color: isActive ? "#f97316" : "#8e8ea0" }} />
